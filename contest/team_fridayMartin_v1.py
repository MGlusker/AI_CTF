...
class BaseCaptureAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    self.legalPositions = self.findLegalPositions(gameState)
    ...
    BaseCaptureAgent.jointInference.initialize(self.ourTeamAgents, self.opponentAgents, gameState, self.legalPositions, self)
  ...
  def chooseAction(self, gameState):
    ...
    print "totalPrunes:", self.totalPrunes
  ...
  def getActionAlphaBeta(self, gameState, dists, currentAgentIndex):
    ...
    successorsAndInfo = []
    for action in ourLegalActions: #get successors from legalActions
    
      try: 
        child = gameState.generateSuccessor(currentAgentIndex, action)
        successorsAndInfo.append((self.evaluationFunction(child), child, action))
      except: 
        print "Max in minimax - Martin Exception"
        continue


    #sortedSuccessors = successorsAndInfo.sort(key = lambda x: x[0])  
   
    exp = lambda tup : tup[0]
    sortedSuccessors = sorted(successorsAndInfo, key = exp, reverse = True)


    cutSuccessors = sortedSuccessors[0:self.forwardPrune]
    
    sortedActions = []
    ourSuccessorsEvalScores = []
    for evalScore, child, action in cutSuccessors:
      ...
      if(v > beta) or (self.timesUp and self.timesUpfeatureOn):
        self.totalPrunes += 1
        break
    ...
    print "time", float(time.time() - self.start)
    if len(ourSuccessorsEvalScores)==0: 
      print sortedSuccessors
      return 'Stop'

    return sortedActions[ourSuccessorsEvalScores.index(max(ourSuccessorsEvalScores))]
  ...
  def maxRecursiveHelper(self, gameState, depthCounter, currentAgentIndex, alpha, beta, DEPTH):
    ...
    if float(time.time() - self.start) > float(0.975) and self.timesUpfeatureOn:
      self.timesUp = True
  ...
  def minRecursiveHelper(self, gameState, depthCounter, currentAgentIndex, alpha, beta, DEPTH):
    ...
    exp = lambda tup : tup[0]
    sortedSuccessors = sorted(successorsAndInfo, key = exp, reverse = False) #reserve should be false
    cutSuccessors = sortedSuccessors[0:self.forwardPrune]
  ...
  def getOffensiveEnemyClosenessScore(self, gameState): 
    ...
    if onMySide:
      ...
      if max(ourScaredTimes) == 0:
        ...
        #  offensiveEnemyClosenessScore = 1000.0
        #
        #else:
        #  offensiveEnemyClosenessScore = 100.0 * (1.0/closestEnemyDistance)
...
#######################
  ...
  def getDefensiveWeights(self, gameState):
    # defensiveFoodScore is positive
    # capsuleScore is positive
    # numInvadersScore is negative if we have invaders, positive if we don't
    # enemyClosenessScore is negative
    # socreOfGame is negative if losing, positive if winning
    Weights = {"defensiveFoodScore": 1000, "defensiveEnemyClosenessScore": 1000, "numInvadersScore": -25000,  "defensiveCapsuleScore": 1, "scoreOfGame": 1000}# "goHomeScore": 1# "defensiveCapsuleScore": 1, "scoreOfGame": 1000}#{"defensiveFoodScore": 1000}#, "capsuleScore": 10, "numInvadersScore": 100, "enemyClosenessScore": 100000, "scoreOfGame": 1}
