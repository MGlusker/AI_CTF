...
class BaseCaptureAgent(CaptureAgent):
  ...
  def getActionAlphaBeta(self, gameState, dists, currentAgentIndex):
    ...
    self.shallowDepth = 4 #In terms of moves, not plies
    self.maxDepth = 8
    self.A = 1.0
    self.B = 0.0
    self.sigma = 1 #maaayyybbee
    self.percentile = 1.5
  ...
  def maxRecursiveHelper(self, gameState, currentAgentIndex, depthCounter, alpha, beta):
    ...
    #cutoff test
    # if(depthCounter == self.shallowDepth):
    #   return self.evaluationFunction(gameState)
    ...
    if(depthCounter == self.maxDepth):
      ...
      bound = float(percentile * sigma + beta - B) / float(A)
      if (minRecursiveHelper(gameState, currentAgentIndex+1, depthCounter+1, bound-1, bound) >= bound )
         return beta;

      bound = float(percentile * sigma + alpha - B) / float(A)
      if (minRecursiveHelper(gameState,  currentAgentIndex+1, depthCounter+1, bound-1, bound) >= bound )
         return alpha;
  ...
  def minRecursiveHelper(self, gameState, currentAgentIndex, depthCounter, alpha, beta):
  ...
  def getOffensiveWeights(self, gameState):
    ...
    Weights = {"offensiveEnemyClosenessScore": 1, "offensiveCapsuleScore": 1}#{"foodScore": 1000, "capsuleScore": 100, "offensiveEnemyClosenessScore": 10, "scoreOfGame": 1} #"capsuleScore": 10, "offensiveEnemyClosenessScore": 10, "scoreOfGame": 1000}
...
#######################
  ...
  def getDefensiveFeatures(self, currentGameState):
    ...
    #features["defensiveCapsuleScore"] = capsuleScore
  ...
  def getDefensiveWeights(self, gameState):
    ...
    # socreOfGame is negative if losing, positive if winning
    Weights = {"defensiveFoodScore": 1, "defensiveEnemyClosenessScore": 1000, "numInvadersScore": 1}#{"defensiveFoodScore": 1000}#, "capsuleScore": 10, "numInvadersScore": 100, "enemyClosenessScore": 100000, "scoreOfGame": 1}
